    ...
    registrationConfirmationUser: async (confirmationCode: string): Promise<Result<WithId<TUser> | null>> => {
        ...
        if(!userByCode) {
            return {
                ...
                extensions: [{ field: 'confirmationCode', message: 'Not found user by this confirmation code' }],
            };
        }
        ...
        if(userByCode.emailConfirmation.isConfirmed === true) {
            return {
                ...
                extensions: [{ field: 'isConfirmed', message: 'this code already been applied' }],
            };
        }
    ...
    registrationEmailResending: async (email: string): Promise<Result<WithId<TUser> | null>> => {
        ...
        if(existingUser.emailConfirmation.isConfirmed === true) {
            return {
                ...
                extensions: [{ field: 'isConfirmed', message: 'User already Confermed' }],
            };
        }

        const { user: updatedUser } = await usersRepository.updateConfirmationCode(email);
        ...
        if(!updatedUser) {
            return {
                ...
                extensions: [{ field: 'email', message: 'User was not updated' }],
            };
        }
